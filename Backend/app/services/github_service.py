...
async def get_commit_stats(client: httpx.AsyncClient, owner: str, repo: str, path: str) -> dict:
    ...
    response = await client.get(url, headers=HEADERS, params={"path": path, "per_page": 100})
    response.raise_for_status()
...
async def get_file_bug_label(client: httpx.AsyncClient, owner: str, repo: str, path: str) -> int:
    url = f"{GITHUB_API_BASE}/repos/{owner}/{repo}/commits"
    response = await client.get(url, headers=HEADERS, params={"path": path, "per_page": 50})
    response.raise_for_status()

    for commit in response.json():
        if BUG_KEYWORDS.search(commit["commit"]["message"]):
            return 1
    return 0
