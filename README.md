...
An end-to-end ML system that analyzes GitHub repositories and ranks files by how likely they are to contain bugs — combining commit-history patterns with AST-extracted code complexity metrics.
...
> **Note on evaluation:** the model has not yet been benchmarked against a held-out, labeled bug dataset (e.g. issue-linked commits or a tool like BugZilla). Current precision/recall figures are not established — this is the top item on the improvement list below before any accuracy claim should be made publicly.
...
// Response
{
  "total_files": 24,
  "high_risk_files": [
    { "file": "auth.py", "risk_score": 0.87, "risk_level": "High" },
    { "file": "db_utils.py", "risk_score": 0.61, "risk_level": "Medium" }
  ]
}
```
```
...
- **Benchmark against a real labeled dataset** (e.g. BugZilla or issue-linked commits) to establish actual precision/recall, instead of relying on proxy labels.
- Measure and publish real pipeline latency across repos of varying size.
